...
const HOW_IT_WORKS = [
  { step: '01', icon: <Leaf size={22} className="text-[#3d7a20]" />, title: 'Harvested at Dawn', desc: 'Our farmers handpick fruits at peak ripeness every morning before sunrise — no carbide, no chemicals, no shortcuts.' },
  { step: '02', icon: <Award size={22} className="text-[#3d7a20]" />, title: 'Quality Checked', desc: 'Each lot is graded and inspected at the cooperative centre in Tenkasi for size, colour, and freshness before packing.' },
  { step: '03', icon: <Truck size={22} className="text-[#3d7a20]" />, title: 'Packed & Dispatched', desc: 'Wrapped in banana leaves or eco-boxes and loaded into refrigerated vehicles by mid-morning for same-day dispatch.' },
  { step: '04', icon: <MapPin size={22} className="text-[#3d7a20]" />, title: 'Delivered to Your Door', desc: 'Orders reach customers across Tamil Nadu by dusk — farm to home in under 12 hours.' },
]
    ...
    <main className="min-h-screen bg-[#faf9f4]">
      ...
      <section className="bg-gradient-to-br from-[#0c1d2b] via-[#3d7a20] to-[#4fb8b2] text-white px-4 py-20 relative overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          <div>
            <p className="text-[#f5821f] text-xs font-bold tracking-widest uppercase mb-4">Est. 1987 · Tenkasi, Tamil Nadu</p>
      ...
      <section className="py-16 sm:py-24 px-4">
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <div>
              <p className="text-[#3d7a20] text-xs font-bold tracking-widest uppercase mb-3">Who We Are</p>
              ...
              <div className="grid grid-cols-3 gap-4">
                {[
                  ...
                  <div key={s.label} className="text-center bg-[#fdf4e8] rounded-2xl py-5 px-3">
      ...
      <section className="py-16 sm:py-24 px-4 bg-white border-t border-gray-100">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-12">
            <p className="text-[#3d7a20] text-xs font-bold tracking-widest uppercase mb-3">The Process</p>
      ...
      <section className="py-16 sm:py-24 px-4">
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div>
              <p className="text-[#3d7a20] text-xs font-bold tracking-widest uppercase mb-3">Where We Are</p>
              ...
              <div className="space-y-3">
                {[
                  { icon: <MapPin size={16} className="text-[#3d7a20]" />, text: '42C/1A Rail Nagar Road, Tenkasi – 627 811, Tamil Nadu' },
                  { icon: <Phone size={16} className="text-[#3d7a20]" />, text: '+91 7094402579' },
                  { icon: <Users size={16} className="text-[#3d7a20]" />, text: '240+ partner farmers across Tenkasi & Shencottai taluks' },
                ].map((item, i) => (
                  ...
                  <div key={r.region} className="flex items-center gap-4">
                    <div className="flex-1">
                      ...
                      <div className="mt-1.5 h-1.5 bg-white/10 rounded-full overflow-hidden">
                        <div className="h-full bg-[#f5821f] rounded-full" style={{ width: `${(r.farmers / 80) * 100}%` }} />
                      </div>
                    </div>
                  </div>
                ))}
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-5">
            {TEAM.map((m) => (
              <div key={m.name} className="bg-[#faf9f4] rounded-2xl border border-gray-100 p-5 text-center">
                <div className="w-14 h-14 bg-[#fdf4e8] rounded-full flex items-center justify-center mx-auto mb-3">
                  <Users size={22} className="text-[#3d7a20]" />
                </div>
                <p className="font-semibold text-gray-900 text-sm">{m.name}</p>
                <p className="text-xs text-[#3d7a20] font-medium mt-0.5">{m.role}</p>
